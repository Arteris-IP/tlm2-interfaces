...
void axi_target_pe_b::end_of_elaboration() { clk_if = dynamic_cast<sc_core::sc_clock*>(clk_i.get_interface()); }
...
void axi_target_pe_b::b_transport(payload_type& trans, sc_time& t) {
    ...
    if(auto* clk = dynamic_cast<sc_core::sc_clock*>(i)) {
        t += clk->period() * latency;
    }
}
...
void axi_target_pe_b::setup_callbacks(fsm_handle* fsm_hndl) {
    ...
    fsm_hndl->fsm->cb[BegPartRespE] = [this, fsm_hndl]() -> void {
        ...
        if(!send_resp_fifo.nb_write(std::make_tuple(fsm_hndl, BegPartRespE)))
            SCCERR(SCMOD)<<"too many outstanding transactions";
    };
    fsm_hndl->fsm->cb[EndPartRespE] = [this, fsm_hndl]() -> void {
        fsm_hndl->trans->is_read() ? rd_resp.post() : wr_resp.post();
        auto size = get_burst_lenght(fsm_hndl->trans) - 1;
        fsm_hndl->beat_count++;
    ...
    fsm_hndl->fsm->cb[BegRespE] = [this, fsm_hndl]() -> void {
        // scheduling the response
        if(!send_resp_fifo.nb_write(std::make_tuple(fsm_hndl, BegRespE)))
            SCCERR(SCMOD)<<"too many outstanding transactions";
    };
    fsm_hndl->fsm->cb[EndRespE] = [this, fsm_hndl]() -> void { fsm_hndl->trans->is_read() ? rd_resp.post() : wr_resp.post(); };
}

void axi::pe::axi_target_pe_b::operation_resp(payload_type& trans, bool sync){
    auto e = axi::get_burst_lenght(trans)==0 || trans.is_write()? axi::fsm::BegRespE:BegPartRespE;
    schedule(e, &trans, SC_ZERO_TIME);
}

void axi::pe::axi_target_pe_b::send_resp_thread() {
    std::tuple<fsm::fsm_handle*, axi::fsm::protocol_time_point_e> entry;
    while(true) {
        // waiting for responses to send
        wait(send_resp_fifo.data_written_event());
        while(send_resp_fifo.nb_read(entry)) {
            ...
            // wait to get ownership of the response link
            fsm_hndl->trans->is_read() ? rd_resp.wait() : wr_resp.wait();
