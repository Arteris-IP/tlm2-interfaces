...
void axi_target_pe::end_of_elaboration() { clk_if = dynamic_cast<sc_core::sc_clock*>(clk_i.get_interface()); }
...
void axi_target_pe::setup_callbacks(fsm_handle* fsm_hndl) {
    ...
    fsm_hndl->fsm->cb[BegReqE] = [this, fsm_hndl]() -> void {
        if(!fsm_hndl->beat_count && max_outstanding_tx.get_value() &&
           outstanding_cnt[fsm_hndl->trans->get_command()] > max_outstanding_tx.get_value()) {
            ...
            if(!fsm_hndl->beat_count)
                getOutStandingTx(fsm_hndl->trans->get_command())++;
            auto latency = fsm_hndl->trans->is_read() ? get_cci_randomized_value(rd_addr_accept_delay)
                                                      : get_cci_randomized_value(wr_data_accept_delay);
            if(latency)
    ...
    fsm_hndl->fsm->cb[EndReqE] = [this, fsm_hndl]() -> void {
        ...
        if(fw_o.get_interface())
            ...
            auto latency = operation_cb                 ? operation_cb(*fsm_hndl->trans)
                           : fsm_hndl->trans->is_read() ? get_cci_randomized_value(rd_resp_delay)
                                                        : get_cci_randomized_value(wr_resp_delay);
    ...
    fsm_hndl->fsm->cb[EndRespE] = [this, fsm_hndl]() -> void {
        ...
        if(stalled_tx[cmd]) {
            ...
            auto latency =
                trans->is_read() ? get_cci_randomized_value(rd_addr_accept_delay) : get_cci_randomized_value(wr_data_accept_delay);
            if(latency)
