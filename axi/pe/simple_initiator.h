...
public:
    ...
    /**
     * @brief the default snoop latency between request and response phase. Will be overwritten by the return of the callback function (if
     * registered)
     * @todo this is a hack and should be fixed
     */
    unsigned snoop_latency{1};

    void add_protocol_cb(axi::fsm::protocol_time_point_e e, std::function<void(payload_type&)> cb) {
...
protected:
    ...
    std::array<std::function<void(payload_type&)>, axi::fsm::CB_CNT> protocol_cb;
};
/**
