    ...
    void initialize_streams() {
        ...
        if(isRecordingBlockingTxEnabled() && !b_streamHandle) {
            ...
            if(enableTimedTracing.get_value()) {
                b_streamHandleTimed = new tx_fiber((full_name + "_bl_timed").c_str(), "[TLM][ace][b][timed]", m_db);
        ...
        if(isRecordingNonBlockingTxEnabled() && !nb_streamHandle) {
            ...
            if(enableTimedTracing.get_value()) {
                nb_streamHandleTimed = new tx_fiber((full_name + "_nb_timed").c_str(), "[TLM][ace][nb][timed]", m_db);
...
template <typename TYPES> void ace_lwtr<TYPES>::b_transport(typename TYPES::tlm_payload_type& trans, sc_core::sc_time& delay) {
...
template <typename TYPES> void ace_lwtr<TYPES>::b_snoop(typename TYPES::tlm_payload_type& trans, sc_core::sc_time& delay) {
    if(!b_streamHandleTimed) {
...
tlm::tlm_sync_enum ace_lwtr<TYPES>::nb_transport_fw(typename TYPES::tlm_payload_type& trans, typename TYPES::tlm_phase_type& phase,
    ...
    if(nb_streamHandleTimed) {
        nb_ace_rec_entry rec(mm::get().allocate(), phase, reinterpret_cast<uint64_t>(&trans), h,
                             (phase == tlm::BEGIN_RESP || phase == axi::BEGIN_PARTIAL_RESP));
...
template <typename TYPES>
tlm::tlm_sync_enum ace_lwtr<TYPES>::nb_transport_bw(typename TYPES::tlm_payload_type& trans, typename TYPES::tlm_phase_type& phase,
                                                    sc_core::sc_time& delay) {
    if(!isRecordingNonBlockingTxEnabled()) {
        if(checker) {
...
template <typename TYPES> void ace_lwtr<TYPES>::nbtx_cb() {
...
template <typename TYPES> bool ace_lwtr<TYPES>::get_direct_mem_ptr(typename TYPES::tlm_payload_type& trans, tlm::tlm_dmi& dmi_data) {
 ...
 * \param end_addr is the end address of the memory area being invalid
 */
template <typename TYPES> void ace_lwtr<TYPES>::invalidate_direct_mem_ptr(sc_dt::uint64 start_addr, sc_dt::uint64 end_addr) {
    if(!(m_db && enableDmiTracing.get_value())) {
